...
OCR_LANG = 'en'
SCORE_THRESHOLD = 0.0
HORIZ_NMS_THRESHOLD = 0.80
VERT_NMS_THRESHOLD = 0.30
...
class TabularOCR:
    ...
    def convert_to_format(self, table_data, output_path, export_format):
        for table_id, (boxes, texts, vert_lines, horiz_lines, vert_boxes, horiz_boxes) in enumerate(table_data):
            ...
            if export_format.lower() == 'csv':
                ...
                logging.warning(f"Unsupported export format '{export_format}'. Skipping table {table_id} export.")
    ...
    def non_max_suppression(self, horiz_boxes, vert_boxes, probabilities, image):
        ...
        horiz_out = cv2.dnn.NMSBoxes(
            ...
            nms_threshold=HORIZ_NMS_THRESHOLD,
            eta=1.0,
            top_k=1000
        )
        ...
        vert_out = cv2.dnn.NMSBoxes(
            ...
            probabilities,
            score_threshold=SCORE_THRESHOLD,
            nms_threshold=VERT_NMS_THRESHOLD,
            eta=1.0,
            top_k=1000
        )
    ...
    def extract(self, output_path='output', export_format='csv'):
        try:
            ...
            for table_image in table_images:
                output = self.preprocess_image(table_image)
                output = self.detect_and_recognize_text(output)
            ...
            if export_format.lower() == 'csv':
                ...
                logging.warning("Unsupported export format. Please choose 'csv' or 'excel'.")

        except Exception as e:
            logging.error(f"An error occurred: {e}")

if __name__ == "__main__":
    input_path = "tabularocr\simple-table.png"
    ocr = TabularOCR(input_path)
